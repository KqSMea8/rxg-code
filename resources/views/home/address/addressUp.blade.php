...
<div class="member-right fr">
            ...
            <div class="member-switch clearfix" id='cc'>
                <span id='one'>编辑收获地址</span>
                <span id='two'>所有内容必填</span>
            </div>
            <div>
            <form action="{{url('address/addressUpdate')}}" method="post" enctype="multipart/form-data">
                {{csrf_field()}}
                <center>
                    <table id ='three'> 
                        ...
                        <tr>
                            ...
                            <td id="area">
                                <select name="province" id="provinces" class="select-1">
                                    @foreach($area as $k => $v)
                                    <option value="{{$v->id}}" id="provinces" {{$info['province'] == $v['id'] ? 'selected' : ''}}>{{$v->areaName}}</option>
                                    @endforeach
                                </select>
                        ...
                        <tr>
                            ...
                            <td>
                               <textarea id='text' name='street' placeholder="请输入详细地址信息,如道路、小区、楼栋号、单元等信息" style="huerreson:expression(this.width=this.scrollWidth);width:348px" value='{{$info['street']}}'>{{$info['street']}}</textarea>
                            </td>
                        </tr>
                        <tr>
                            <td align='right'><span>*</span>邮政编码:</td>
                            <td>
                                <input type="text" name='email' placeholder="如您不清楚邮递区号,请填写000000" id='email' onblur="emai()" value='{{$info['email']}}' style="width:348px">
                            </td>
                        </tr>
                        <tr>
                            <td align='right'><span>*</span>收货人姓名:</td>
                            <td>
                                <input type="text" name='user_name' placeholder="长度不超过25个字符" id='userName' onblur='user()' value='{{$info["user_name"]}}' style="width:348px">
                            </td>
                        </tr>
                        <tr>
                            ...
                            <td>
                                ...
                                <input type="text" name='tell' placeholder="电话号码必须填" id='tell' onblur='te()' style="width:233px" value='{{$info['tell']}}'>
                            </td>
                        </tr>
                        <tr>
                            <td align='right'></td>
                            <td>
                                <input type="submit"  value='保存' class="btn btn-danger btn-xs"/>
                            </td>
                        </tr>
                    </table>
                </center>
        ...
        $.ajax({
            ...
            success:function(res){
                ...
                $(res.data).each(function(k,v){
                    if(v.id=="{{$info['city']}}"){
                        $('#city').append("<option value="+v.id+" selected>"+v.areaName+"</option>");
                    }else{
        ...
        $("#distruct").empty();
          $.ajax({
            ...
            success:function(res){
                ...
                $(res.data).each(function(k,v){
                    console.log(v);
                    if(v.id=="{{$info['distruct']}}"){
                        $("#distruct").append("<option value="+v.id+" selected>"+v.areaName+"</option>");
