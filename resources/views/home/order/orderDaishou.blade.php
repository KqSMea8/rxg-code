...
<li class="cur"><a href="{{url('order/daishou')}}?id=1">待收货</a></li>
</ul>
</div>
...
<span class="gr2"><a href="#" id="goodsName">{{$v->goodsName}}</a></span>
...
<div class="ci5">
   ...
   @if($v->orderStatus==1) 配送中
  
   @endif
</p> 
<p><a href="{{url("order/logistics")}}"><font color="blue">物流跟踪</font></a></p> <p><a href="{{url('order/orderContent')}}?id={{$v->orderId}}">订单详情</a></p></div>
<div class="ci5 ci8"><p>剩余15时20分</p> <p><a href="{{url('order/orderHuo')}}?id={{$v->orderId}}" class="member-touch">确认收货</a> </p> <p><a href="{{url('order/resetOrder')}}?id={{$v->orderId}}">取消订单</a> </p></div>
</div>
</li>
@endforeach
