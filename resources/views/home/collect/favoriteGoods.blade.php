					...
					<div class="member-vessel">
						...
						@foreach($data as $k=>$v)
											...
											<input type="checkbox" value="{{$v['goodsId']}}">
												<a href="#"><img src="{{URL::asset('/')}}{{$v['goodsImg']}}" title="" width="125" height="125"></a>
												<p>{{$v['goodsName']}}</p>
												<p>￥{{$v['marketPrice']}}</p>
											</li>
...
<script>
	$(document).on('click',"#btn",function(e){
		...
		$.ajax({
			url:"{{url('collect/delFavoriteGoods')}}",
			...
			success:function(res){
				if (res.code==1) {
					location.href="{{url('collect/favoriteGoods')}}";
				}
			}
		})
	})
